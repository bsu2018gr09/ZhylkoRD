...
int main() {
	...
	c.r = 128; c.g = 16; c.b = 77;// очень неудобно менять цвет т.к. эти числа тут, а потом в строке 31 а потом еще в 41
	double dr = (255 - 128) / 80;// очень неудобно менять второй цвет
	double dg = (49 - 16) / 80;
	double db = (99 - 77) / 80;
	for (int i = 1; i <= h; ++i) {
		for (int j = 1; j <= w; ++j) {
